...
public class IflytekSpeechEvalProvider implements SpeechEvalProvider {
    ...
    public SpeechEvalResult evaluate(byte[] audioFile, String referenceText, String language, String sentenceId, String userId, String contentType, String originalFilename) {
        ...
        try {
            ...
            log.info("Speech evaluation request endpoint={} referenceText={} pcmBytesLength={} audioBase64Length={} aue={} auf={}",
                    vendor.getEndpoint(), referenceText, audioBytes.length, audioBase64.length(), aue, auf);

            JsonNode root;
            if (vendor.getEndpoint().startsWith("wss://") || vendor.getEndpoint().startsWith("ws://")) {
                root = webSocketClient.evaluate(audioBytes, referenceText, language, sentenceId, userId, vendor, aue, auf);
            } else {
                root = evaluateByHttp(audioBase64, referenceText, language, sentenceId, userId, vendor, aue, auf);
            }
